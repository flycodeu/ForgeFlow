...
升级前先从托盘执行“停止后台并退出”，确认安装目录中没有 ForgeFlow 或其 Node 后台进程，再运行新版安装包。安装器识别已有 ForgeFlow 目录并允许就地覆盖；若仍有进程，当前保护脚本会尝试强制结束，因此不能将此过程称为平滑停止。其他非空目录会被拒绝；安装包没有自动更新或代码签名。业务数据目录与程序目录分离，升级前仍应备份数据库并核对配置路径。机器需已有 Microsoft Edge WebView2 Runtime。
