...
const CORE_SECTIONS = [
  '1. 目标', '2. 设计依据', '3. 范围', '4. 不包含范围', '5. 使用场景 / 运行场景', '6. 能力 / 行为设计',
  '7. 关键规则', '8. 依赖与关联', '9. 异常、边界与恢复', '10. 实现影响', '11. 验证与验收条件', '12. 未决事项',
];
...
function buildTemplate(input: GuidanceInput, profile: ProjectProfile) {
  ...
  return `# ${input.featureName}｜详细设计

> 项目类型：${PROFILES[profile].label}｜结构：Core Sections + Adaptive Sections

# 1. 目标

说明这个 Feature 要产生的可观察结果，以及成功后谁获得什么能力。
...
export function buildFeatureDesignGuidance(input: GuidanceInput): FeatureDesignGuidance {
  ...
  return {
    ...
    guardrails: [
      ...
      'Capability 必须包含足以指导实现的输入、行为、规则、失败与验收条件。',
