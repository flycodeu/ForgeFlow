...
${input.capabilitySummary || '填写这一操作对使用者产生的结果。'}

### 输入字段

按真实输入列字段、类型、必填、约束；无输入时写明触发条件。
...
列关键处理顺序、权限/并发校验、失败反馈与恢复。

### 接口与数据

列实际入口、请求与响应，读写的数据对象、字段及关系；没有 HTTP 或数据库时写真实协议和状态载体。

### 验收要点

列主路径与关键失败路径的可观察结果；不要把尚未运行的测试写成通过。

### 依据与未决

${evidence(input)}

仅保留影响本操作的已确认事实；未定参数注明待定。${PROFILE_SECTIONS[profile].sections.join('、')}等技术细节只在实际涉及且有证据时补充。
`,
    guardrails: [
      '一张 Capability 卡只写一项操作；不要把功能总览、原始 DDL、哈希或实施状态复制进设计。',
      '模板是写作提示，提交前必须替换提示句；不适用章节可省略，未验证的结果应明确标注。',
      '技术章节由 designProfile、项目上下文和 Capability 语义共同决定，designProfile 不是内容限制。',
      '不得为无 HTTP、无数据库或无 UI 项目虚构 Controller、REST API、数据库表或页面。',
      '字段、接口、数据关系和未决项要能追到当前来源；缺证据时不要补造具体参数。',
    ],
  };
}
