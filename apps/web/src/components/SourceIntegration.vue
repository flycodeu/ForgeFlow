...
<template>
  <section class="source-workspace">
    ...
    <aside v-if="showHelp" class="source-help">
      ...
      <strong>关于本地路径</strong>
      <p>路径按工作环境登记；只有获得该目录权限的 Codex 或 Claude Code 才能读取。保存路径不等于已分析。</p>
    </aside>
    ...
    <div v-if="sources.length" class="source-table">
      ...
      <template v-for="source in sources" :key="source.id">
        ...
        <article v-if="expandedSourceId === source.id" class="source-detail">
          ...
          <div><button type="button" @click.stop="addEnvironment(source)">＋ 添加环境位置</button><button type="button" @click.stop="openAnalysis([source.id])">请求分析</button></div>
        </article>
      </template>
    </div>
    ...
    <section class="analysis-section">
      ...
      <article v-if="selectedAnalysis" class="analysis-detail">
        ...
        <div class="prompt-actions"><button type="button" @click="copyPrompt('codex')">复制 Codex 提示词</button><button type="button" @click="copyPrompt('claude')">复制 Claude Code 提示词</button></div>
    ...
    <div v-if="showSourceDialog" class="source-dialog-backdrop" @click.self="showSourceDialog = false">
      <section class="source-dialog" role="dialog" aria-modal="true" aria-labelledby="source-dialog-title">
        ...
        <form @submit.prevent="saveSource">
          ...
          <label>本地源码目录<input v-model="sourceDraft.localRoot" maxlength="1024" required placeholder="D:\Projects\video-api" /><small>支持粘贴中文和空格路径。</small></label>
    ...
    <div v-if="showAnalysisDialog" class="source-dialog-backdrop" @click.self="showAnalysisDialog = false">
      <section class="source-dialog analysis-dialog" role="dialog" aria-modal="true" aria-labelledby="analysis-dialog-title">
        ...
        <form @submit.prevent="requestAnalysis">
          ...
          <small class="dialog-hint">创建后状态为“等待 AI”，需由外部 Codex 或 Claude Code 读取。</small>
...
<style scoped>
.source-workspace{display:grid;gap:20px;max-width:1440px;margin:0 auto;padding-bottom:40px}.source-heading{display:flex;min-height:48px;align-items:center;justify-content:space-between;gap:20px}.source-heading h1{margin:0;color:var(--ink);font-size:28px;line-height:1.2}.source-actions{display:flex;gap:9px}.help-button{min-height:36px;padding:0 10px;border:0;background:transparent;color:var(--muted);font-size:13px}.source-help{position:relative;padding:14px 42px 14px 16px;border:1px solid var(--line);background:#f8faf9}.source-help strong{font-size:14px}.source-help p{margin:5px 0 0;color:var(--muted);font-size:13px;line-height:1.55}.source-help button{position:absolute;top:8px;right:10px;border:0;background:transparent;color:var(--muted);font-size:20px}.source-table,.analysis-section{border:1px solid var(--line);background:#fff}.source-table-head,.source-row{display:grid;grid-template-columns:minmax(180px,1.15fr) minmax(250px,1.7fr) 110px 110px 170px 24px;align-items:center;gap:14px;padding:0 16px}.source-table-head{min-height:42px;border-bottom:1px solid var(--line);background:#f6f8f9;color:var(--muted);font-size:13px;font-weight:700}.source-row{width:100%;min-height:58px;border:0;border-bottom:1px solid #e8ecef;background:#fff;color:var(--ink);font-size:14px;text-align:left}.source-row:hover{background:#f8faf9}.source-row>code{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.source-row>b{text-align:right}.source-name{display:grid;gap:3px;min-width:0}.source-name strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:15px}.source-name small{overflow:hidden;color:var(--muted);font-size:13px;text-overflow:ellipsis;white-space:nowrap}.status-badge{justify-self:start;display:inline-flex;min-height:26px;align-items:center;padding:0 8px;border-radius:999px;background:#eef1f3;color:#55646d;font-size:13px;font-weight:700;white-space:nowrap}.status-badge[data-tone="success"]{background:#e2f1eb;color:#17614e}.status-badge[data-tone="waiting"]{background:#fff1d9;color:#865b12}.status-badge[data-tone="danger"]{background:#f9e5e7;color:#9b3945}.source-detail{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:18px;padding:14px 16px 16px 30px;border-bottom:1px solid var(--line);background:#f8faf9}.source-detail dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px 24px;margin:0}.source-detail dl>div{display:grid;grid-template-columns:100px minmax(0,1fr);gap:10px}.source-detail dt{color:var(--muted);font-size:13px}.source-detail dd{min-width:0;margin:0;font-size:14px;overflow-wrap:anywhere}.source-detail dd small{display:block;margin-top:3px;color:var(--muted);font-size:13px}.source-detail>div{display:flex;align-items:flex-start;gap:8px}.source-detail button,.prompt-actions button{min-height:34px;padding:0 10px;border:1px solid var(--line-strong);background:#fff;color:#315e4b;font-size:13px}.source-empty{display:flex;min-height:100px;align-items:center;justify-content:space-between;padding:20px;border:1px solid var(--line);background:#fff}.source-empty p,.empty-line{margin:0;color:var(--muted);font-size:14px}.analysis-section>header{display:flex;min-height:48px;align-items:center;justify-content:space-between;padding:0 16px;border-bottom:1px solid var(--line)}.analysis-section h2{margin:0;font-size:20px}.analysis-section>header span{color:var(--muted);font-size:13px}.empty-line{padding:20px}.analysis-table>button{display:grid;width:100%;grid-template-columns:100px minmax(170px,1fr) 150px 120px 180px 20px;align-items:center;gap:12px;min-height:48px;padding:0 16px;border:0;border-bottom:1px solid #e8ecef;background:#fff;color:var(--ink);font-size:14px;text-align:left}.analysis-table>button:hover,.analysis-table>button.active{background:#f5f8f7}.analysis-table time{color:var(--muted);font-size:13px}.analysis-table i{font-style:normal;text-align:right}.analysis-detail{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px 20px;padding:16px;border-top:1px solid var(--line);background:#f8faf9}.analysis-detail>div:first-child{display:grid;gap:4px}.analysis-detail>div:first-child strong{font-size:15px}.analysis-detail>div:first-child span{color:var(--muted);font-size:14px}.prompt-actions{display:flex;gap:8px}.analysis-detail details{grid-column:1/-1}.analysis-detail summary,.scope-editor summary{cursor:pointer;color:#40535c;font-size:13px;font-weight:700}.analysis-detail pre{max-height:180px;margin:10px 0 0;overflow:auto;padding:12px;background:#17232a;color:#dce6e9;font:13px/1.55 var(--mono);white-space:pre-wrap}.source-dialog-backdrop{position:fixed;z-index:80;inset:0;display:grid;place-items:center;padding:24px;background:rgba(18,28,31,.5)}.source-dialog{display:grid;grid-template-rows:auto minmax(0,1fr);width:min(960px,calc(100vw - 48px));max-height:80vh;border:1px solid var(--line-strong);background:#fff;box-shadow:0 24px 80px rgba(12,24,28,.25)}.source-dialog>header{display:flex;min-height:60px;align-items:center;justify-content:space-between;padding:0 22px;border-bottom:1px solid var(--line)}.source-dialog h2{margin:0;font-size:20px}.source-dialog>header button{border:0;background:transparent;color:var(--muted);font-size:24px}.source-dialog form{display:grid;gap:14px;min-height:0;overflow-y:auto;padding:20px 22px}.source-dialog label{display:grid;gap:6px;color:#40515a;font-size:14px;font-weight:700}.source-dialog input,.source-dialog textarea,.source-dialog select{box-sizing:border-box;width:100%;min-height:40px;padding:8px 10px;border:1px solid var(--line-strong);background:#fff;color:var(--ink);font:14px/1.45 inherit}.source-dialog textarea{min-height:76px;resize:vertical}.source-dialog label small,.dialog-hint{color:var(--muted);font-size:13px;font-weight:400}.field-pair{display:grid;grid-template-columns:1fr 1fr;gap:14px}.scope-editor{padding:12px;border:1px solid var(--line)}.scope-editor .field-pair{margin-top:12px}.source-dialog footer{display:flex;justify-content:flex-end;gap:9px;padding-top:4px}.source-dialog fieldset{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:0;padding:12px;border:1px solid var(--line)}.source-dialog legend{padding:0 6px;font-size:14px;font-weight:700}.source-check{display:flex!important;align-items:center;gap:9px!important;min-height:40px;padding:8px;background:#f7f9fa}.source-check input{width:auto;min-height:auto}.source-check code{margin-right:8px;color:#245a47}.dialog-hint{display:block}.analysis-dialog{width:min(760px,calc(100vw - 48px))}@media(max-width:1200px){.source-table-head,.source-row{grid-template-columns:minmax(170px,1fr) minmax(220px,1.4fr) 90px 105px 24px}.source-table-head span:nth-child(5),.source-row>span:nth-child(5){display:none}.source-detail{grid-template-columns:1fr}.analysis-table>button{grid-template-columns:90px minmax(150px,1fr) 125px 115px 20px}.analysis-table time{display:none}}@media(max-width:800px){.source-heading{align-items:flex-start;flex-direction:column}.source-table-head{display:none}.source-row{grid-template-columns:minmax(0,1fr) auto auto 20px;padding-block:10px}.source-row>code,.source-row>span:nth-child(3){display:none}.source-detail dl{grid-template-columns:1fr}.analysis-table>button{grid-template-columns:85px minmax(0,1fr) auto 20px}.analysis-table>button>span:nth-child(3){display:none}.field-pair,.source-dialog fieldset{grid-template-columns:1fr}.analysis-detail{grid-template-columns:1fr}.prompt-actions{justify-content:flex-start}}
</style>
