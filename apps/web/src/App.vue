...
const engineeringCompleteness = computed(() => {
  ...
  return detail.features.map((feature) => {
    ...
    const operationCount = detail.capabilities.filter((item) => item.featureId === feature.id
      && detail.specifications.some((spec) => spec.capabilityId === item.id && spec.latestRevisionId)).length;
    return { feature, operationCount, kinds: required.map((kind) => ({ kind, label: labels[kind] ?? kind, exists: existing.has(kind) })) };
  });
});
    ...
    <template v-if="appView === 'workspace'">
      ...
      <div v-else-if="currentProject && projectDetail" class="console-layout">
        <aside class="sidebar" :class="{ open: mobileNavOpen }">
          ...
          <div class="sidebar-footer"><span class="health-dot" :class="{ offline: health !== '服务正常' }"></span><span>{{ health }}</span></div>
        </aside>
    ...
    <div v-if="showCreateTokenModal" class="dialog-backdrop" @click.self="closeCreateTokenModal">
      <section class="dialog token-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="token-dialog-title">
        ...
        <div v-else class="token-modal-body">
          <p class="connection-security-note">关闭窗口后密钥不再显示，请及时复制接入命令。</p>
